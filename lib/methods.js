Meteor.methods({
  ...
  lessonAdding: function (schedule, lessonnr, query) {
    ...
    console.log("add: ", schedule, +lessonnr, query, Rows.update( { schedule: schedule, lessonnr: +lessonnr }, { $push: query } ));
  },
  lessonRemoving: function (schedule, lessonnr, query) {

    console.log("remove: ", schedule, +lessonnr, query, Rows.update( { schedule: schedule, lessonnr: +lessonnr }, { $pull: query } ));
  }
});
