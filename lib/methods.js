Meteor.methods({
  ...
  lessonAdding: function (schedule, lesson, query) {

    Rows.update( { schedule: schedule, lesson: lesson }, { $push: query } );
  },
  lessonRemoving: function (schedule, lesson, query) {

    Rows.update( { schedule: schedule, lesson: lesson }, { $pull: query } );
  }
});
